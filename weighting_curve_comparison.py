...
f3, W3 = plot_frequency_weighting(
    low_gain=0.3, f_low=0.5, f_mid_start=2.0, f_mid_end=5.0, f_flat_end=16.0,
    # plotting domain
    fmin=0.016, fmax=63.0, n_points=2000,
    # shape controls
    log_ramp=True,             # ramp (f_mid_start → f_mid_end) straight on log–log
...
plt.loglog(f1, W1, color="C0", ls=":", linewidth=2, label="ISO 2631 ($W_k$) ($w=w_0=0.4$)") # Baseline
plt.loglog(f2, W2, color="C1", ls="--", linewidth=2,  label="Lower Bound (faster pacing) ($w=0.1$)") # Lower Bound
plt.loglog(f3, W3, color="C3", ls="--", linewidth=2, label="Upper Bound (slower pacing) ($w=0.3$)") # Upper Bound
plt.xlabel("$f$ (Hz)", fontsize=16)
plt.ylabel("Weighting Factor", fontsize=16)
...
# plt.ylabel("Weighting Factor", fontsize=15)
