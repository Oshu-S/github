...
def analyze_vibration(file_path, trial, low_gain=0.4, f_low=0.5, f_mid_start=2.0, f_mid_end=5.0, f_flat_end=16.0, tail_exponent=1.0, log_ramp=True, show_knots=True, plot=True):
    ...
    if plot:
        ...
        plt.plot(t, acceleration, label="$a(t)$")
        plt.plot(t_rms, rms_running_unweighted, label="$a(t_0)$")
        plt.plot(t, weighted_signal, label="$a_W(t)$", color="y")
        plt.plot(t_rms, rms_running_weighted, label="$a_W(t_0)$", color="r")
    ...
    if plot:
        plt.figure(figsize=(6.5, 4.5))
        mpos = f_psd > 0
        plt.loglog(f_psd[mpos], Pxx[mpos], label="$S_{a}(f)$")
        plt.loglog(f_psd[mpos], Pxx_w[mpos], label="$S_{a_W}(f)$")
